...
/* ------------   引擎设计说明   ------------
 ...
 引擎的设计初衷是, 通过分层架构, 让各层的职责单一化(这就是 "关注点隔离思想" ),
 各层职责单一化的好处是 :
 1)容易进行单元测试;
 2)耦合程度降到最低;
 3)可以让团队中擅长不同技术方向的程序员们各尽所能(比如有擅长做UI的, 有擅长做模块设计的);
 4)leader可以在上总控全局, 哪里不满意, 就可以单独的替换那部分代码;
 ...
 在我的引擎中, 最大程度的保证了业务层开发的单一性, 在业务层通过 "外观模式" 获取一个统一的API接口 AppNetworkEngineSingleton,
 ...
 调用 AppNetworkEngineSingleton 提供的方法, 发起一个网络接口的访问后, 会直接返回 id<INetRequestHandle>,
 这个就是给业务层提供的, 本次网络请求的操作句柄, 通过这个句柄可以查询本次网络请求的运行情况, 并且可以取消本次网络请求.
 AppNetworkEngineSingleton 提供的方法, 永远不会返回一个nil对象, 当发生错误时, 会返回一个 NetRequestHandleNilObject 对象,
...
// 文件上传/下载 异步http响应监听 (在UI线程中)
// 成功
typedef void (^FileAsyncHttpResponseListenerInUIThreadSuccessedBlock)(NSString *filePath, NSString *responseBody);
// 失败
typedef void (^FileAsyncHttpResponseListenerInUIThreadFailedBlock)(ErrorBean *error);
// 进度
typedef void (^FileAsyncHttpResponseListenerInUIThreadProgressBlock)(double progress);
 ...
 *
 */
@interface SimpleNetworkEngine : NSObject
