...
+ (long long) folderSizeAtPath1:(NSString*) folderPath{
  ...
  while ((fileName = [childFilesEnumerator nextObject]) != nil){
    ...
    if ([self fileSizeAtPath1:fileAbsolutePath] != [self fileSizeAtPath2:fileAbsolutePath]){
      PRPLog(@"%@, %lld, %lld", fileAbsolutePath,
...
+ (void)deleteFolderAtPath:(NSString *)folderPath isDeleteRootFolder:(BOOL)isDeleteRootFolder {
  ...
  do {
    ...
    // TODO : iOS 删除文件夹, 不需要像 android那样逐级删除子文件夹后, 才能删除顶层文件夹, 可以直接删除一个完整的文件夹, 一次性就OK了
    if (isDeleteRootFolder) {
      [fileManager removeItemAtPath:folderPath error:NULL];
    } else {
      NSError *errorOUT = nil;
      NSArray *contents = [fileManager contentsOfDirectoryAtPath:folderPath error:&errorOUT];
      if (errorOUT != nil) {
        break;
      }
